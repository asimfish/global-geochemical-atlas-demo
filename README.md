...
- `index.html`：门户首页（hero + 赛题 + 运行产物 + 工作原理「六条设计承诺」+ 快速上手（挂载命令与可替换槽位的默认 Prompt）+ 三类真实运行证据 + 科学发现层 + 数据库对比 + 演示视频 + 材料下载），可滚动、自解释、响应式；含 Open Graph / Twitter 卡片元信息与 SVG favicon。
- `en.html`：首页的完整英文版，结构与 `index.html` 逐节对应；顶栏「EN / 中文」互切，选择会记忆在 `localStorage`（也可用 `?lang=en` / `?lang=zh` 强制指定）；两页互设 `hreflang`。
- `assets/site.css` / `assets/site.js`：两页共用的样式与脚本（入场动画、统计数字滚动、代码复制按钮、≤1140px 折叠菜单、语言偏好记忆）。
- `slides.html`：14 页评审幻灯片（支持滚轮 / 方向键 / 触摸翻页）；第 11–12 页为科学发现层与同类数据库对比。
- `research/`：科学发现层产物——五篇论文 PDF 全部成稿（P1 10 页 / P2–P5 各 9 页，合计 46 页、24 图 9 表，均过 a–g 质量审计与语料库措辞打磨）、五篇论文路线图、同类数据库对比（含数字出处）。
...
12. 科学发现层：discovery_mode 自动选题长出五篇论文（五篇全部成稿、合计 46 页、24 图 9 表、a–g 审计全过，全部实测数字）；
13. 同类数据库对比：与 EarthChem/GEOROC、USGS NGDB、GEOTRACES 的定位差异（更全面 / 更可靠 / 更统一）；
14. 收尾：Global Geochemical Atlas，统一生成赛题五项输出。
