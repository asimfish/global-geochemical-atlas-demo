...
- `index.html`：门户首页（hero + 五项交付 + 两次真实运行证据 + 科学发现层 + 数据库对比 + 演示视频 + 材料下载），可滚动、自解释、响应式。
- `slides.html`：14 页评审幻灯片（支持滚轮 / 方向键 / 触摸翻页）；第 11–12 页为科学发现层与同类数据库对比。
- `research/`：科学发现层产物——五篇论文 PDF 全部成稿（P1 10 页 / P2 8 页 / P3 8 页 / P4 8 页 / P5 8 页，合计 42 页、20 图 9 表，均过 a–g 质量审计）、五篇论文路线图、同类数据库对比（含数字出处）。
...
12. 科学发现层：discovery_mode 自动选题长出五篇论文（五篇全部成稿、合计 42 页、20 图 9 表、a–g 审计全过，全部实测数字）；
